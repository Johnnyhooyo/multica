...
**做**:把一个很窄的收件箱事件白名单转发到 IM 私信,并让该推送可被回复——回复注入到对应 issue,由既有的评论触发器唤醒 agent。
...
一个硬编码写 `done` 的命令**比今天更弱**。agent 在那一轮里做的不只是翻状态,还有收尾工作(汇入提交、核对依赖);直接写状态会跳过这些。命令的确定性换不来这个损失。

而且既然自然语言回复已经被验证可用,再引入一套精确匹配、大小写敏感的命令词法,是在解决一个不存在的问题。
